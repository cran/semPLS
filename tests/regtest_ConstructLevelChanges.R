...
clcData <- read.table("./ConstructLevelChanges/clcData.csv", header=T)
sm <- as.matrix(read.table("./ConstructLevelChanges/clcStruc.csv", header=T))
mm <- as.matrix(read.table("./ConstructLevelChanges/clcMeas.csv", header=T))
...
clcBoot <- bootsempls(clc, nboot=7, method="ConstructLevelChanges", verbose=F)
